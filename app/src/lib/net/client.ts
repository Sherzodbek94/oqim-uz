...
export interface OnlinePlayer {
  ...
  cash: number;
  salary: number;
  assets: { id: string; title: string; kind: string; icon: string; price: number; monthlyCashflow: number; employees?: number; constructionLeft?: number; monthlyRevenue?: number; monthlyOperatingCosts?: number; operatingCostParts?: Asset['operatingCostParts']; businessModel?: Asset['businessModel']; operations?: Asset['operations']; market?: Asset['market'] }[];
  loansCount: number;
  children: number;
  escaped: boolean;
  bankrupt: boolean;
